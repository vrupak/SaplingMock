...
export function DocumentTimelineItem({ item, styles, alignment, fullWidth = false }: DocumentTimelineItemProps) {
  const { Icon } = styles
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <Badge className="bg-gray-100 text-gray-700 border-none text-[10px] font-bold tracking-wide uppercase">
                DOCUMENT
              </Badge>
              <span className="font-medium text-sapling">{item.documentType}</span>
            </div>
            <p className="text-sm font-medium text-gray-900 mb-1">{item.fileName}</p>
